...
class CustomerAccountAdminForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for role in GenericPartyRole.objects.all():
            choices += (("%d::%s::%s"%(role.id, role.__class__.__name__, role),"%s : %s"%(role.name,role.party)),)
...
class CustomerAdmin(GenericPartyRoleAdmin):
    ...
    fields = ('valid_to','name','party', 'customer_number', 'customer_status')
    readonly_fields = ('name', 'party_content_type', 'party_object_id')
    inlines = [AccountInline,PhysicalContactInline, EmailContactInline, TelephoneNumberInline]
...
class CustomerAccountAdmin(admin.ModelAdmin):
    list_display = ('customer', 'account_number', 'name','account_status','credit_limit')
...
class CustomerOrderAdmin(admin.ModelAdmin):
    list_display = ('interaction_date', 'interaction_status', 'customer_order_type')
